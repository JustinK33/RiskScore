...
- `MissingIndicator(features="all")` as its own branch over the same columns, so the indicators never reach the scaler (audit B33). Missingness is still a feature - a borrower with no `revol_util` has no revolving account, which is not a borrower at the median - but standardizing that feature is what broke serving. A column with one gap in a 629-row fit gives its indicator an sd near 0.016, so an applicant who omitted an optional field arrived 60 standard deviations out: the reason codes attributed `-5.0` log-odds to `revol_bal: null`, more than every real signal combined, and the probability fell from 0.070 to 0.004 on nothing but an absent field. Unscaled, the contribution of a missing value is the coefficient itself. `features="all"` rather than the default `"missing-only"` so the block follows the *spec* - a column with no gaps in train yields a constant zero whose coefficient regularizes to zero, and a column that starts arriving with gaps after deployment has somewhere to say so.
- `StandardScaler` after the imputer, on the values only.
